...
The public-facing frontend is built with **React + Vite** (see the `portal/` directory), with HMR during development and ESLint for code quality.
...
For the React frontend portal, install dependencies inside `portal/`:
```bash
cd portal
npm install
```
...
### Launching the Frontend (React + Vite)
